...
class freq_normaliser() :
	...
	def plot_graph_all_tokens(self, path_dist_of_tokens, all_tokens):
	...
	def plot_graph_methods(self, list_of_methods, method_names, outlier_method_names, remaining_method_names):
		...
		outlier_method_values = []
		remaining_method_values = []
		o = []
		r = []
		for i in range(0, len(list_of_methods)):
			if list_of_methods[i] in outlier_method_names:
				outlier_method_values.append(list_of_methods[i])
				o.append(i)
			else:
				remaining_method_values.append(list_of_methods[i])
				r.append(i)
		x_out = np.array(outlier_method_values)
		y_out = np.arange(0, len(outlier_method_values))
		...
		for x, y in zip(x_out, y_out):
			...
			#label = "{:.2f}".format(y)
			label = method_names[o[i]]
			i = i + 1
			plt.annotate(label, (x, y), textcoords="offset points", xytext = (0, 10), ha = "center")
			

		x_rem = np.array(remaining_method_values)
		y_rem = np.arange(0, len(remaining_method_values))
